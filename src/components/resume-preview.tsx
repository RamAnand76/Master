...
const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div className="mb-6">
        <h2 className="text-sm font-semibold text-primary/80 border-b-2 border-primary/20 pb-1 mb-3 uppercase tracking-widest">{title}</h2>
        {children}
    </div>
);
  ...
  return (
    <div className="p-8 text-sm text-foreground bg-background font-body min-h-[calc(100vh-122px)]">
      <header className="text-center mb-8">
        {personalDetails?.name && <h1 className="text-4xl font-bold tracking-tight">{personalDetails.name}</h1>}
        ...
        <Section title="Experience">
            {experience.map(exp => (
                <div key={exp.id} className="mb-4 last:mb-0">
                    ...
                    <ul className="mt-2 text-muted-foreground list-disc pl-5 space-y-1 whitespace-pre-wrap text-sm">
          ...
          <Section title="Skills">
              <div className="flex flex-wrap gap-2">
                  {skills.filter(s => s.name).map(skill => (
                      <span key={skill.id} className="bg-secondary text-secondary-foreground rounded-md px-2.5 py-1 text-xs font-medium">
                          {skill.name}
                      </span>
                  ))}
              </div>
